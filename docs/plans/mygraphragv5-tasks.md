...
Modify `src/ManagedCode.GraphRag/Indexing/IndexingPipelineRunner.cs` using an OVERLOAD (backward compatible — do NOT change the existing signature, because callers passing a CancellationToken positionally as the 2nd arg would break). Keep the existing `RunAsync(GraphRagConfig config, CancellationToken ct = default)`. Extract the current body into a private `RunCoreAsync(GraphRagConfig config, IWorkflowCallbacks callbacks, CancellationToken ct)` and replace the hardcoded `NoopWorkflowCallbacks.Instance` (line 27) with the `callbacks` parameter. The existing public method calls `RunCoreAsync(config, NoopWorkflowCallbacks.Instance, ct)`; add a new public overload `RunAsync(GraphRagConfig config, IWorkflowCallbacks callbacks, CancellationToken ct = default)` that calls `RunCoreAsync(config, callbacks, ct)`. Verify no overload ambiguity for existing 1-arg / (config, CancellationToken) call sites, and that existing callers/tests still compile unchanged.
